...
function TaskCard({ task, done }: { task: SummaryTask; done: boolean }) {
  ...
  return (
    <div className="bg-[#0f0e0d] rounded-xl overflow-hidden">
      <button
        onClick={() => hasSessions && setOpen(v => !v)}
        className={`w-full flex items-center gap-3 px-4 py-3 text-left ${hasSessions ? 'hover:bg-[#191817]' : ''} transition-colors`}
      >
        <span className={`text-xs shrink-0 w-3 ${done ? 'text-[#5f7f58]' : 'text-[#6f695f]'}`}>{done ? '✓' : '·'}</span>
        <span className={`flex-1 text-sm truncate ${done ? 'text-[#c8c8c8]' : 'text-[#7a7367]'}`}>{task.title}</span>
        {task.time_seconds > 0 && (
          <span className="text-[11px] text-[#6f695f] font-mono shrink-0">{formatSeconds(task.time_seconds)}</span>
        )}
        {hasSessions && <span className="text-[#4a463f] text-[10px] shrink-0 ml-1">{open ? '▲' : '▼'}</span>}
      </button>
      {open && hasSessions && (
        <div className="border-t border-[#1e1e1e] divide-y divide-[#1e1e1e]">
          {task.sessions.map(s => (
            <div key={s.id} className="px-4 py-2.5 flex gap-3 items-start">
              <span className="text-[11px] text-[#453f37] font-mono shrink-0 mt-0.5 whitespace-nowrap">
                {formatTime(s.started_at)}{s.ended_at ? ` → ${formatTime(s.ended_at)}` : ''}
              </span>
              <span className="text-[11px] text-[#6f695f] font-mono shrink-0 mt-0.5">{formatSeconds(s.duration_actual)}</span>
              {s.note && <span className="text-[11px] text-[#9c958a] italic flex-1 leading-snug">{s.note}</span>}
            </div>
          ))}
        </div>
      )}
  ...
  return (
    <>
      <div className="px-6 pt-5 pb-2 shrink-0">
        <div className="text-xs text-[#e0a458] font-semibold tracking-widest uppercase mb-1">Шаг 2 из 2</div>
        <h2 className="text-xl font-bold text-[#ece7df] mb-0.5">Что завтра?</h2>
        <p className="text-sm text-[#6f695f] capitalize">{tomorrowStr}</p>
      </div>
      ...
      <div className="px-6 pb-2 overflow-y-auto flex-1 min-h-0">
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-left transition-colors ${
                        active ? 'bg-[#e0a458]/20 border border-[#e0a458]/40' : 'bg-[#0f0e0d] border border-transparent hover:bg-[#191817]'
                      }`}
                    >
                      <span className={`w-4 h-4 rounded shrink-0 border flex items-center justify-center text-[10px] ${
                        active ? 'border-[#e0a458] bg-[#e0a458] text-white' : 'border-[#38342e]'
                      }`}>
                        {active ? '✓' : ''}
                      </span>
        ...
        {available.length === 0 && (
          <p className="text-sm text-[#6f695f] text-center py-8">Нет незавершённых задач</p>
        )}
      </div>

      <div className="px-6 py-4 shrink-0 border-t border-[#2a2723] flex gap-2">
        <button onClick={onSkip} className="flex-1 py-2.5 bg-raised hover:bg-border-strong rounded-xl text-sm text-[#9c958a] transition-colors">
          Пропустить
        </button>
        <button
...
export default function EveningSummaryModal({ tasks, directions, onClose, onLater }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6" style={{ background: 'radial-gradient(80% 60% at 50% 0%, rgba(27,25,23,0.97) 0%, rgba(16,15,14,0.98) 100%)' }}>
      ...
      <div className="relative z-10 animate-rise-in bg-overlay border border-border-strong rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">

        {step === 1 ? (
          <>
            {/* Animated bars */}
            <div className="flex items-end gap-1 px-6 pt-5 pb-1 h-14 shrink-0" style={{ background: 'linear-gradient(180deg, #24201a 0%, #292623 100%)' }}>
              {bars.map((h, i) => (
                <div key={i} className="flex-1 rounded-t-sm" style={{
                  height: `${h * 100}%`,
                  backgroundColor: `hsl(${18 + i * 3}, ${42 + h * 14}%, ${38 + h * 18}%)`,
            ...
            <div className="px-6 pt-4 pb-2 shrink-0">
              <div className="text-xs text-[#4a463f] font-semibold tracking-widest uppercase mb-1">Шаг 1 из 2</div>
              <h2 className="text-xl font-bold text-[#ece7df] mb-0.5">Итог дня</h2>
              <p className="text-sm text-[#6f695f] capitalize">{today}</p>
            </div>

            <div className="px-6 py-3 shrink-0">
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-[#0f0e0d] rounded-xl p-3">
                  <div className="text-2xl font-bold text-[#ece7df]">{doneCount}</div>
                  <div className="text-xs text-[#6f695f] mt-0.5">задач выполнено</div>
                </div>
                <div className="bg-[#0f0e0d] rounded-xl p-3">
                  <div className="text-2xl font-bold text-[#ece7df]">{formatSeconds(timeSeconds)}</div>
                  <div className="text-xs text-[#6f695f] mt-0.5">время в работе</div>
                </div>
              </div>
            </div>
              ...
              <div className="px-6 pb-2 overflow-y-auto flex-1 min-h-0">
                <div className="space-y-1.5">
                  ...
                  {workedTasks.length > 0 && doneTasks.length > 0 && (
                    <div className="section-label pt-2 pb-1">В работе</div>
                  )}
            ...
            <div className="px-6 py-4 shrink-0 border-t border-[#2a2723] flex gap-2">
              <button onClick={onLater} className="flex-1 py-2.5 bg-raised hover:bg-border-strong rounded-xl text-sm text-[#9c958a] transition-colors">
                Позже
              </button>
              <button onClick={() => setStep(2)} className="flex-1 py-2.5 bg-accent hover:bg-accent-light rounded-xl text-sm text-[#1c1610] font-medium transition-colors">
                Закрыть день →
              </button>
            </div>
          </>
