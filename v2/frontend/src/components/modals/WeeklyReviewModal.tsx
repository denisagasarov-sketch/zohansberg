  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6" style={{ background: 'radial-gradient(80% 60% at 50% 0%, rgba(27,25,23,0.97) 0%, rgba(16,15,14,0.98) 100%)' }}>
      ...
      <div className="relative z-10 animate-rise-in bg-overlay border border-border-strong rounded-2xl w-full max-w-2xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
        ...
        {/* Header bar */}
        <div className="px-6 pt-5 pb-3 shrink-0" style={{ background: 'linear-gradient(180deg,#1a2030 0%,#1b1a18 100%)' }}>
          <div className="text-xs text-[#e0a458] font-semibold tracking-widest uppercase mb-1">
            Шаг {step} из 2 · Еженедельный обзор
          </div>
          <h2 className="text-xl font-bold text-[#ece7df]">
            {step === 1 ? 'Итог недели' : 'Что на следующей неделе?'}
          </h2>
          <p className="text-sm text-[#6f695f] mt-0.5">{weekRange}</p>
        </div>
          ...
          <>
            ...
            <div className="px-6 pb-2 overflow-y-auto flex-1 min-h-0 space-y-3">
              ...
              {data?.done_tasks?.length > 0 && (
                <div>
                  <div className="section-label mb-2">Выполнено</div>
            ...
            <div className="px-6 py-4 shrink-0 border-t border-[#2a2723] flex gap-2">
              ...
              <button onClick={() => setStep(2)} className="flex-1 py-2.5 bg-accent hover:bg-accent-light rounded-xl text-sm text-[#1c1610] font-medium transition-colors">Планировать неделю →</button>
            </div>
          </>
        ) : (
          <>
                      ...
                      {dt.map(task => {
                        ...
                        return (
                          <button key={task.id} onClick={() => toggle(task.id)}
                            className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-left transition-colors ${active ? 'bg-[#e0a458]/20 border border-[#e0a458]/40' : 'bg-[#0f0e0d] border border-transparent hover:bg-[#191817]'}`}>
                            <span className={`w-4 h-4 rounded shrink-0 border flex items-center justify-center text-[10px] ${active ? 'border-[#e0a458] bg-[#e0a458] text-white' : 'border-[#38342e]'}`}>{active ? '✓' : ''}</span>
            ...
            <div className="px-6 py-4 shrink-0 border-t border-[#2a2723] flex gap-2">
              ...
              <button onClick={handleSavePlan} className="flex-1 py-2.5 bg-accent hover:bg-accent-light rounded-xl text-sm text-[#1c1610] font-medium transition-colors">
