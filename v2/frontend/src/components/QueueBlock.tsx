  ...
  return (
    ...
    >
      <div className="px-4 pt-3 pb-1">
        <div className="section-label mb-2">Следом</div>
      </div>

      {queueTasks.length === 0 ? (
        isDragOver ? (
          <div className="px-4 pb-3 text-xs text-[#e0a458]">Отпустите, чтобы добавить в очередь</div>
        ) : (
          <div className="px-4 pb-3 text-xs text-[#9c958a]">Очередь пуста — добавьте задачи из направлений</div>
        )
      ) : (
          ...
          {queueTasks.map((task, idx) => {
            ...
            return (
              <li
                ...
                className={`group relative flex items-center gap-3 px-4 py-2 cursor-pointer hover:bg-raised/60 transition-colors ${draggingId === task.id ? 'opacity-40' : ''} ${focusDimmed(task) ? 'opacity-30 blur-[3px]' : ''}`}
              ...
              >
                ...
                {c ? (
                  ...
                  <div className="absolute left-0 top-0 bottom-0 w-0.5 bg-[#e0a458]/40 rounded-r" />
                ) : null}
                <span className="text-[#4a463f] text-[10px] cursor-grab select-none shrink-0">⠿</span>
                <span className="text-[#4a463f] text-xs font-mono w-4 shrink-0">{idx + 1}</span>
                <PriorityBadge priority={task.priority} onChange={v => onPriorityChange(task.id, v)} />
                <span className="flex-1 text-sm text-[#ece7df] truncate">{task.title}</span>
                {task.notes && <span className="text-[#9c958a] shrink-0" title={task.notes}><Icon name="note" size={12} /></span>}
                {task.recurrence && <span className="text-[10px] text-[#e0a458]/60 shrink-0" title="Повторяющаяся задача">↺</span>}
                {task.deadline && (
                  <span className="text-[10px] text-[#9c958a] shrink-0">
                ...
                <button
                  ...
                  className="opacity-0 group-hover:opacity-100 text-[#6f695f] hover:text-[#ece7df] text-base leading-none shrink-0 transition-opacity px-0.5"
