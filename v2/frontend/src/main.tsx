...
class RootBoundary extends React.Component<{ children: React.ReactNode }, { crashed: boolean }> {
  ...
  render() {
    if (this.state.crashed) {
      return (
        <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 14, background: '#141312', color: '#ece7df', fontFamily: 'system-ui, sans-serif' }}>
          <div style={{ fontSize: 14, color: '#a49d90' }}>Приложение перезапускалось несколько раз подряд</div>
          <button
            onClick={() => { try { sessionStorage.removeItem(RELOAD_KEY) } catch { /* ignore */ } location.reload() }}
            style={{ padding: '8px 18px', fontSize: 13, borderRadius: 6, border: '1px solid #38342e', background: '#2a2723', color: '#ece7df', cursor: 'pointer' }}
          >
            Перезагрузить
          </button>
        </div>
