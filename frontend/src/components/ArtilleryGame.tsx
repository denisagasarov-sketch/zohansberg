  ...
  const render = (c: CanvasRenderingContext2D) => {
    ...
    for (const wm of st.worms) {
      ...
      if (isActive && !st.over) {
        ...
        c.fillStyle = wm.team === 'you' ? '#ffd24c' : '#8fb0ff'
    ...
    // particles
    for (const p of st.parts) { c.fillStyle = p.water ? (p.life > 0.5 ? '#bfe3ff' : '#7fb8e8') : (p.life > 0.5 ? '#000' : '#555'); const sz = 2 + p.life * 2; c.fillRect(p.x - sz / 2, p.y - sz / 2, sz, sz) }
    c.restore()
  }
