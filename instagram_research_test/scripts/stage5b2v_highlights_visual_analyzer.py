        ...
        response = client.chat.completions.create(
            ...
            messages=[{
                ...
                "content": [
                    *image_content,
                    {"type": "text",
                     "text": "Это финальные кадры хайлайта. Найди CTA — явный призыв к действию с глаголом: написать в директ, перейти по ссылке, записаться, купить, оставить заявку и т.п.\nЕсли CTA не найден — ответь только: not_found\nЕсли найден — ответь только дословным текстом CTA с кадра, максимум 80 символов. Никаких объяснений, только текст или not_found."},
                ],
            }],
            max_tokens=MAX_TOKENS_TARGETED,
            temperature=0.0,
