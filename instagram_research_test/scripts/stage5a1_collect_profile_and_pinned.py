...
Два actor call:
  1. apify/instagram-scraper  resultsType=details  → profile fields
  2. apify/instagram-scraper  resultsType=posts     → isPinned detection

НЕ запускает OpenAI.
НЕ скачивает media.
НЕ использует resultsType=profiles.

Запуск через stage5a1_run_local.py:
...
def field_ok(value, source_ref, notes=""):
    return {
        "value":       value,
        "data_status": "ok",
...
def field_partial(value, source_ref, notes=""):
    return {
        "value":       value,
        "data_status": "partial",
        "source_ref":  source_ref,
...
def field_manual_needed(notes=""):
    return {
        "value":       None,
        "data_status": "manual_needed",
...
def build_profile_summary(details_item: dict, run_ts: str, run_meta: dict) -> dict:
    ...
    url_type = detect_url_type(ext_url_val or "")

    def wrap_int(val, label):
        if val is not None:
            return field_ok(val, src)
        return field_manual_needed(f"{label} not returned by actor")
    ...
    ext_url_wrapped = field_ok(ext_url_val, src) if ext_url_val else field_manual_needed("externalUrl not in actor output")
    ...
    return {
        ...
        "profile_url":          field_ok(PROFILE_URL, "config"),
        ...
        "external_url_type":    field_ok(url_type, "rule_based") if url_type != "none" else field_missing("external_url empty or not matched"),
        ...
        "raw_source":           "data/raw/stage5a1_profile_details_raw.json",
    }

# ---------------------------------------------------------------------------
# Build bio_analysis.json — rule-based only, no OpenAI
# ---------------------------------------------------------------------------
...
def bio_rule_based(bio_text: str) -> dict:
    ...
    # --- niche / target / result / positioning ---
    # Only pull literal lines — mark as partial (rule-based inference)
    # Lines 1–2 often describe niche/role
    ...
    return {
        "niche": field_partial(niche_line, src, "First bio line — may be role/niche, rule-based") if niche_line else field_manual_needed("Cannot extract niche from bio automatically"),
        "target_audience": field_partial(target_line, src, "Line matching помогаю/работаю с/для — rule-based") if target_line else field_manual_needed("Cannot extract target_audience from bio automatically"),
        "result_promise": field_partial(result_line, src, "Line matching result keywords — rule-based") if result_line else field_manual_needed("Cannot extract result_promise from bio automatically"),
        "positioning": field_manual_needed("Positioning requires human interpretation — use OpenAI in Stage 5D or fill manually"),
        "social_proof": field_partial(sp_lines if sp_lines else None, src, "Lines with digits or social_proof keywords — rule-based") if sp_lines else field_manual_needed("No social proof lines detected in bio"),
        "trust_arguments": field_manual_needed("Trust arguments require human or OpenAI interpretation"),
        "cta_text": field_partial(cta_text_val, src, "Arrow or CTA-keyword line — rule-based") if cta_text_val else field_manual_needed("No CTA line detected in bio"),
        "cta_destination": None,  # filled by caller from external_url
    }


def build_bio_analysis(profile_summary: dict, run_ts: str) -> dict:
    ...
    if bio_text:
        analysis = bio_rule_based(bio_text)
    else:
        none_field = field_manual_needed("bio_text missing — cannot analyse")
    ...
    # cta_destination from external_url
    if ext_url:
        analysis["cta_destination"] = field_ok(ext_url, "profile_summary.external_url")
    else:
        analysis["cta_destination"] = field_manual_needed("No external_url in profile — fill manually")
...
def build_pinned_posts_index(posts_items: list, run_ts: str) -> dict:
    total_checked = len(posts_items)

    # Check if isPinned field exists in any item
    ...
    if not has_field:
        return {
            ...
            "pinned_posts":     [],
            "raw_source":       "data/raw/stage5a1_posts_for_pinned_raw.json",
        }
    ...
    def wrap_post_field(val, key_name):
        if val is not None and val != "":
            return field_ok(val, f"posts_actor.{key_name}")
        return field_missing(f"{key_name} not in actor output")
    ...
    for pos, item in enumerate(pinned_items, start=1):
        ...
        pinned_list.append({
            ...
            "is_pinned":      field_ok(True, "posts_actor.isPinned"),
    ...
    if pinned_count == 0:
        manual_needed = True
        notes = "actor supports isPinned but no pinned posts found in first 30 posts"
    else:
        manual_needed = False
        notes = ""
...
def collect(client) -> dict:
    ...
    # --- Call 1: details ---
    print("\n[1/2] Running details mode ...")
    details_items = []
    try:
        run1 = client.actor(ACTOR_ID).call(run_input=PAYLOAD_DETAILS)
        actual_calls += 1
        rid1 = run1.get("id") or run1.get("defaultDatasetId") or ""
        if rid1:
            run_ids.append(rid1)
        details_items = list(client.dataset(run1["defaultDatasetId"]).iterate_items())
        print(f"  OK — {len(details_items)} item(s)")
        raw1 = [safe_item(i) for i in details_items]
        raw1_path = RAW_DIR / "stage5a1_profile_details_raw.json"
        raw1_path.write_text(json.dumps(raw1, ensure_ascii=False, indent=2), encoding="utf-8")
        print(f"  raw saved: {raw1_path.relative_to(BASE)}")
    except Exception as exc:
        print(f"  FAIL: {exc}")

    details_item = details_items[0] if details_items else {}

    # --- Call 2: posts for pinned ---
    print("\n[2/2] Running posts mode for pinned detection ...")
    posts_items = []
    try:
        run2 = client.actor(ACTOR_ID).call(run_input=PAYLOAD_POSTS)
        actual_calls += 1
        rid2 = run2.get("id") or run2.get("defaultDatasetId") or ""
        if rid2:
            run_ids.append(rid2)
        posts_items = list(client.dataset(run2["defaultDatasetId"]).iterate_items())
        print(f"  OK — {len(posts_items)} item(s)")
        raw2 = [safe_item(i) for i in posts_items]
        raw2_path = RAW_DIR / "stage5a1_posts_for_pinned_raw.json"
        raw2_path.write_text(json.dumps(raw2, ensure_ascii=False, indent=2), encoding="utf-8")
        print(f"  raw saved: {raw2_path.relative_to(BASE)}")
    except Exception as exc:
        print(f"  FAIL: {exc}")
    ...
    profile_summary = build_profile_summary(details_item, run_ts, run_meta)
    bio_analysis    = build_bio_analysis(profile_summary, run_ts)
    pinned_index    = build_pinned_posts_index(posts_items, run_ts)
    stage5a_summary = build_stage5a_summary(profile_summary, bio_analysis, pinned_index, run_ts, run_meta)

    (NORM_DIR / "profile_summary.json").write_text(
        json.dumps(profile_summary, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    (NORM_DIR / "bio_analysis.json").write_text(
        json.dumps(bio_analysis, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    (NORM_DIR / "pinned_posts_index.json").write_text(
        json.dumps(pinned_index, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    (NORM_DIR / "stage5a_summary.json").write_text(
        json.dumps(stage5a_summary, ensure_ascii=False, indent=2), encoding="utf-8"
    )

    print("\n[Normalized outputs saved]")
    print(f"  data/normalized/profile_summary.json")
    print(f"  data/normalized/bio_analysis.json")
    print(f"  data/normalized/pinned_posts_index.json")
    print(f"  data/normalized/stage5a_summary.json")
...
def dry_run():
    ...
    token_ok = bool(token and token.startswith("apify_api_") and token.isascii())

    print("=== Stage 5A-1: DRY RUN ===")
    print(f"Account:    {ACCOUNT}")
    print(f"Actor:      {ACTOR_ID}")
    print(f"Profile URL:{PROFILE_URL}")
    print()
    print(f"APIFY_TOKEN: {'found, format OK (not printed)' if token_ok else 'INVALID or missing'}")
    print()
    print("--- Payload 1: details ---")
    print(json.dumps(PAYLOAD_DETAILS, ensure_ascii=False, indent=2))
    print()
    print("--- Payload 2: posts (pinned detection) ---")
    print(json.dumps(PAYLOAD_POSTS, ensure_ascii=False, indent=2))
    print()
    print("planned_apify_calls: 2")
