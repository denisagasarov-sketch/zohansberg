...
.story-grid { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 8px; }
.story-cell {
    background: #1a252f;
    border-radius: 4px;
    padding: 6px;
...
.batch-header { background: #2c3e50; color: #ecf0f1; padding: 10px 16px; }
...
def section_summary(acc, hl):
    ...
    parts.append('<div class="card"><div class="card-header">Что продает аккаунт</div><div class="card-body">')
    parts.append(f'<p style="font-size:14px;margin-bottom:6px">{h(primary_offer.get("value", "не определено"))}</p>')
    parts.append(render_source_refs(primary_offer.get("source_refs", [])))
    parts.append('</div></div>')

    funnel_roles = acc.get("funnel_roles", {})
    parts.append('<div class="card"><div class="card-header">Роль постов (funnel)</div><div class="card-body">')
    any_role = False
    ...
    parts.append('<div class="card"><div class="card-header">Роль хайлайта</div><div class="card-body">')
    if hl_summary:
    ...
    if hl_roles:
        roles_str = ", ".join(f"{r} ({c})" for r, c in sorted(hl_roles.items(), key=lambda x: -x[1]))
    ...
    parts.append(
        ...
        f'<span class="field-label">Объём</span>'
    ...
    parts.append('<div class="card"><div class="card-header">На чём строится доверие</div><div class="card-body">')
    if trust_mechanics:
    ...
    parts.append('<div class="card"><div class="card-header">Слабые места / пробелы</div><div class="card-body">')
    if weak_spots:
        for ws in weak_spots:
            parts.append(f'<div style="margin-bottom:8px">&#9888; {h(ws.get("spot", ""))}</div>')
    ...
    parts.append('<div class="card"><div class="card-header">Уверенность анализа</div><div class="card-body">')
    parts.append(
        f'<div class="field-row">'
...
def section_posts(posts_data, plan_index):
    ...
    for request_id, post in posts_data:
        ...
        url_link = (
            f'<a href="{h(url)}" target="_blank" style="font-size:11px;color:#2980b9">'
            f'{h(url[:70])}</a>'
            if url else ""
        )
        ...
        parts.append(
            f'<div class="field-row">'
            f'<span class="field-label">Соц. доквo</span>'
            f'<span class="field-value">{field_value_or_warning("Соц. доквo", sp)}</span>'
            f'</div>'
        )

        if cap_facts:
            parts.append('<h4>Факты из caption</h4><ul class="evidence-list">')
            for f_ in cap_facts:
                parts.append(f'<li>{h(f_)}</li>')
            parts.append('</ul>')

        if vis_facts:
            parts.append('<h4>Визуальные факты</h4><ul class="evidence-list">')
            for f_ in vis_facts:
                parts.append(f'<li>{h(f_)}</li>')
            parts.append('</ul>')
        ...
        if limitations:
            ...
            if lim_visible:
                parts.append('<h4>Ограничения</h4><ul class="evidence-list">')
                for lim in lim_visible:
                    parts.append(f'<li style="color:#e67e22">{h(lim)}</li>')
                parts.append('</ul>')

        parts.append('<div class="evidence-block"><h4>Evidence</h4>')
...
def section_highlights(batches_data, hl_plan_index, story_index):
    ...
    for batch_id, batch in batches_data:
        ...
        dss_str = f" · DSS {dss}/10" if dss is not None else ""

        parts.append(f'<div class="card" id="{h(batch_id)}">')
        parts.append('<div class="batch-header">')
        parts.append(f'<h3>Batch {h(batch_idx)} — stories {h(str(story_numbers))}</h3>')
        parts.append(
        ...
        if trust_mechs:
            parts.append('<h4>Механики доверия</h4><ul class="evidence-list">')
            for tm in trust_mechs:
                parts.append(f'<li>{h(tm)}</li>')
            parts.append('</ul>')
        if visible_text:
            parts.append('<h4>Видимый текст в stories</h4><ul class="evidence-list">')
        ...
        # right: story grid
        parts.append('<div><h4>Кадры и stories</h4><div class="story-grid">')
        for i, (sid, snum) in enumerate(zip(story_ids, story_numbers)):
            ...
            parts.append('</div>')
        parts.append('</div></div>')  # story-grid + right col
    ...
    for batch_id, batch in batches_data:
        ...
        for vt in visible_text:
            ...
            if key and key not in seen:
                seen[key] = {"text": str(vt).strip(), "source": batch_id, "roles": main_roles}
...
def section_trust(acc, hl):
    ...
    if not merged:
        ...
        for item in merged.values():
            origins_html = " ".join(f'<code class="ref">{h(o)}</code>' for o in item["origin"])
...
def build_html(body_html, acc, hl):
    ...
    nav_html = "\n".join(f'<a href="{h(href)}">{h(label)}</a>' for href, label in nav_items)
